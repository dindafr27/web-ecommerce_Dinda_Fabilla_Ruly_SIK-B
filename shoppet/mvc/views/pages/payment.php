			...
			<div class="row m-0">
				...
				<div class="col-12 col-md-3 p-0">
					...
					<?php while ($row = mysqli_fetch_array($data["GetFee"])) { ?>
						<p class="float-right mb-0" style="clear: right;"><?php echo $row["name"]; ?>: <?php echo $row["price"]; ?><?php if ($row["name"] == 'delivery') {
																																																													echo '$';
																																																												} else {
																																																													echo '%';
																																																												} ?>
...
<script type="text/javascript">
	...
	function checkoutReviewPrinf() {
		...
		for (let i = 0; i < cart.length; i++) {
			...
			<p class="mb-0 float-left" style="font-size: 80%;margin-top: -2px"> ` + cart[i].quanlity + `</p>		
	...
	function infoCartToDb() {
		...
		for (let i = 0; i < cart.length; i++) {
			...
			<div class="pl-1" style="width: calc(100% - 80px);height: 80px;">
			<p class="font-weight-bold mb-0">` + cart[i].name + `</p>
			<p class="mb-0 float-left" style="font-size: 80%;margin-top: -2px">&nbsp;Quantity :` + cart[i].quanlity + `</p>		
